...
char *idmdll_name = NULL;	//"INNIDM";
				// not sure what it is, turn it off
...
//static inline 
int strip_underscore (const char *name)
{
  if (!opt_rmunder)
    return 0;
...
//static inline 
void put_str(const char *pszName)
{
...
WHAT THE FUCK?!
		...
		sprintf(pachName, "%s%s", pszOrgName, weak_marker);
		}
            else
